...
#inxs = kotel.index
breakpoint()
datetime_index = ( '23.11.2021 21:00',  '23.11.2021 22:00')
...
def import_par_from_KOTELPP(inx, kotel, **par):
       ...
       inx ... datetime_index, tzn. index = ('25.2.2021 18:00', '25.2.2021 19:00')
       kotel ... 'KOTEL PP.LST', tzn. kotel = pd.read_csv('KOTEL PP.LST',engine='python', delimiter=r"\t",   header =[0],skiprows=None, \
       index_col = [0,1])
       **par = {'1st':'KOTEL_CostCnt', '2nd':'KOTEL_CostBnt', '3rd':'KOTEL_CostAnt', '4th':'KOTEL_Pdgnt'}
       ...
       for i in range(0 ,number_of_useful_rows):
              ...
              for k,v in par.items():
                     try:
                         hodnota = float(doh19[i][v])
                     except ValueError:
                         print(f'ValueError while converting float(doh19[i][v]) = {doh19[i][v]}, where i = {i} and v = {v}')
                         #breakpoint()
                         hodnota = float(doh19[i][v].replace(' ',''))
                     value.append(hodnota)
...
vys = import_par_from_KOTELPP(datetime_index,kotel, **par3)

n =0
for k in vys.keys():
       #breakpoint()
       if isinstance(vys[k][0],(int,float)):
              n +=vys[k][0]
       #vys[k]
print(f'n = {n}')
